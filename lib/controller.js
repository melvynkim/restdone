...
const RestdoneScope = require('./scope');
const DataService = require('./data-service');
const utils = require('./utils');

const { requireOptions } = utils;

const ERROR_TYPE = 'restdone';
...
class Controller extends DataService {
  ...
  createScope(controller, transport) {
    const scope = new RestdoneScope(this, this.contextFactory);
    ...
    _.sortBy(this.actions, 'priority').forEach((action) => {
      try {
        if (action.enabled) {
          ...
          const restdoneRouteHandler = async (scope) => {
            try {
          ...
          action.method.forEach((method) => {
            action.transports.forEach((transport) => (
              transport.addRoute(this, method, this.path, action, restdoneRouteHandler)
            ));
          });
        }
      } catch (err) {
...
Controller.ACTIONS = RestdoneScope.ACTIONS;
